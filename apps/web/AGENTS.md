...
```bash
npm run dev          # portless run next dev → https://cmux-diff.localhost
npm run build        # next build (required before running the CLI)
...
**`CMUX_DIFF_REPO` must be set for API routes to resolve the correct repo.**
...
- Comments are stored in `.git/cmux-diff-comments.json` (gitignored in the target repo, not this one)
